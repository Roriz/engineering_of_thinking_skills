...
# Defensive Bug Remediation Lifecycle and Root Cause Fixer

This skill implements the **Defensive Engineering and Bug Remediation Lifecycle** (SPEC-ENG-BUG-0042). It provides a systematic, 6-phase engineering workflow for taking error traces (such as Rollbar JSON error dumps, stack traces, or telemetry reports) and achieving a verified, root-cause fix with regression testing and defensive safeguards.

## Core Engineering Principles and Axioms
...
```mermaid
flowchart TD
    P1["Phase 1: Triage and Data Gathering<br/>(Extract Rollbar JSON or Telemetry)"] --> P2["Phase 2: Stack Trace Deconstruction<br/>(Map origin vs propagation and source maps)"]
    P2 --> P3["Phase 3: Isolation and Reproduction<br/>(MRE, Git Bisect, Delta Debugging)"]
    P3 --> P4["Phase 4: Root Cause Analysis (RCA)<br/>(5 Whys Framework and Causal Matrix)"]
    P4 --> P5["Phase 5: Test-Driven Remediation<br/>(Failing test first, root fix, impact audit)"]
    P5 --> P6["Phase 6: Verification and Safeguards<br/>(Regression check, lint and architecture rules)"]
```

---

### Phase 1: Triage and Telemetry Data Gathering
...
### Phase 2: Stack Trace Deconstruction and Call Chain Analysis
...
2. **Symbolic Resolution and Source Maps**:
...
3. **Traverse Asynchronous and Worker Boundaries**:
...
### Phase 3: Isolation and Deterministic Reproduction
...
### Phase 5: Test-Driven Remediation (TDR) and Side-Effect Audit
...
### Phase 6: Verification, Safeguards, and Sign-Off

1. **Blast Radius Analysis**:
   - Check all callers and dependent modules to ensure zero breaking contract changes.
2. **Add Static Analysis and Architectural Guardrails**:
   - Add lint rules (ESLint, clippy/rustc), schema validations, or architectural constraints to permanently ban the anti-pattern across the codebase.
3. **Execute SOP Sign-Off Checklist**:
   - [ ] **Telemetry Captured**: Stack trace, Rollbar JSON, environment metadata cataloged.
